...
class SpaceRepo():
    ...
    def all(self):
        ...
        spaces = []
        for row in rows:
            space = Space(row["id"], row["property_name"], row["location"], row["beds"], row["property_type"], Decimal(row["price_per_night"]), row["description"], row["image_url"], row["rating"], row["availability"], row["booked_dates"], row["host_id"])
            spaces.append(space)
        return spaces
    ...
    def find_by_id(self, id):
        ...
        hosts = self._connection.execute('SELECT * FROM users WHERE id = %s', [space_object.host_id])
        host = hosts[0]
    ...
    def create(self, space):
        self._connection.execute(
            ...
            [space.property_name, space.location, space.beds, space.property_type, float(space.price_per_night), space.description, space.image_url, space.rating, space.availability, space.booked_dates, space.host_id]
        )
        return None
